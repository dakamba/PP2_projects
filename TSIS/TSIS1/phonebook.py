...
def get_paginated():
    ...
    while True:
        try:
            cur = conn.cursor()
            # Иправлено: используем SELECT вместо callproc для TABLE functions
...
if __name__ == '__main__':
    # Раскомментируй эти строки, если нужно будет пересоздать базу с нуля.
    # Но так как ты уже сделал это в pgAdmin, пока оставим так.
    ...
    while True:
        ...
        if c == '0':
            ...
            if conn:
                try:
                    cur = conn.cursor()
                    # Исправлено: SELECT для функций-таблиц
